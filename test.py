    ...
    def batches(self, n):
        for i in range(0, len(self.image_paths), n):
            images = np.concatenate(
                list(map(self.read_image, self.image_paths[i:i + n])), axis=0)
...
def test_imagenet(model, data_path, val_path, images_path, top_k=5):
    test_data = tf.placeholder(tf.float32, shape=(
        model.batch_size, model.crop_size, model.crop_size, model.channels))
    ...
    with tf.Session() as sesh:
        ...
        for idx, (images, labels) in enumerate(imagenet.batches(model.batch_size)):
            correct += np.sum(sesh.run(top_k_op,
                                       feed_dict={test_data: images, test_labels: labels}))
            count += len(images)
...
def main():
    ...
    if len(args) not in (3, 4):
        print((
            'usage: %s net.params imagenet-val.txt imagenet-data-dir [model-index=0]' % os.path.basename(__file__)))
        exit(-1)
