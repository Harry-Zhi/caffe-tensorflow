...
if PYCAFFE_AVAILABLE:
    ...
    try:
        sys.path.append(
            os.path.join(os.path.dirname(caffe.__file__), 'proto/'))
...
class Node(object):
    ...
    def get_only_parent(self):
        if len(self.parents) != 1:
            raise KaffeError(
                'Node (%s) expected to have 1 parent. Found %s.' % (self, len(self.parents)))
    ...
    def __str__(self):
        ...
        for node in self.topologically_sorted():
            ...
            s.append('{:<20} {:<30} {:>20} {:>20}'.format(
                node.kind, node.name, str(data_shape), str(out_shape)))
        return '\n'.join(s)
...
class DataReshaper(object):
    ...
    def map(self, ndim):
        try:
            ...
            raise KaffeError(
                'Ordering not found for %d dimensional tensor.' % ndim)
    ...
    def reshape(self, graph, replace=True):
        for node in graph.nodes:
            ...
            if (node.kind == NodeKind.InnerProduct) and self.has_spatial_parent(node):
                ...
                fc_order = self.map(2)
                data = data.reshape(
                    (fc_shape[IDX_C_OUT], in_shape[IDX_C], in_shape[IDX_H], in_shape[IDX_W]))
                data = self.transpose(data)
                node.reshaped_data = data.reshape(
                    fc_shape[fc_order[0]], fc_shape[fc_order[1]])
            else:
...
class GraphBuilder(object):
    ...
    def load(self):
        ...
        with open(self.def_path, 'rb') as def_file:
            text_format.Merge(def_file.read().decode('latin-1'), self.params)
        ...
        for layer in layers:
            ...
            for child_name in layer.top:
                ...
                if child_name in graph:
                    ...
                    # by substituting this node wherever the overwritten node
                    # is referenced.
                    node_outputs[child_name] = node
                else:
                    # This is an "implicit" child node: not explicitly
                    # defined in the prototxt, but as a top (output) for some
                    # layer.
